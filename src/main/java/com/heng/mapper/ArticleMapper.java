...
public interface ArticleMapper {
    ...
    * */
    ...
    @Select("SELECT `aid`,`uid`,`title`,`markdown`,`html`,`gmt_post`,`view` FROM `article` ORDER BY `gmt_post` DESC ")
    @Results(value = {@Result(id = true, property = "aid", column = "aid"),
            @Result(property = "uid", column = "uid"),
            @Result(property = "title", column = "title"),
            @Result(property = "markdown", column = "markdown"),
    ...
    * */
    ...
    @Select("SELECT `aid`,`uid`,`title`,`markdown`,`html`,`gmt_post`,`view` FROM `article` ORDER BY `gmt_post` DESC LIMIT #{startIndex},#{count} ")
    @Results(value = {
            @Result(column = "gmt_post", property = "gmtPost", javaType = java.util.Date.class, jdbcType = JdbcType.TIMESTAMP)
    })
    List<Article> listArticleByPage(@Param("startIndex") Integer startIndex, @Param("count") Integer count);
    ...
    * */
    ...
    @Select("SELECT `aid`,`uid`,`title`,`markdown`,`html`,`gmt_post`,`view` FROM `article` ORDER BY `view` DESC LIMIT #{count}")
    @Results(value = {
            @Result(column = "gmt_post", property = "gmtPost", javaType = java.util.Date.class, jdbcType = JdbcType.TIMESTAMP)
    })
    List<Article> viewMost(@Param("count") Integer count);
