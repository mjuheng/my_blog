...
public interface CategoryMapper {
    ...
    * 通过用户id 获取所有分类
    * */
    @Select("SELECT `cid`,`uid`,`category`,`gmt_create` FROM `category`")
    @Results(value = {
    ...
    * */
    ...
    @Delete("<script>DELETE FROM `category` WHERE `cid` IN <foreach collection=\"cidList\" item=\"c\" open=\"(\" separator=\",\" close=\")\" >#{c}</foreach></script>  ")
